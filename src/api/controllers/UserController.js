...
const getSearchAccountUser = async (req, res, next) => {
  ...
  switch (q) {
    case "email":
      ...
      break;
    case "phone_login":
      const exitsPhone = await User.findOne({
        phone_login: req.query.phone_login,
      });
      res.status(200).json({
        exits: !!exitsPhone,
      });
...
const checkLogin = (req, res, next) => {
  ...
  checkEmail
    ...
    : (query["phone_login"] = req.body.phone_login);
  // res.json(query)
  User.findOne(query)
    .then(async (user) => {
      if (user) {
        let comparePass = await ComparePassword(
          req.body.password,
          user.password
        );
        if (comparePass) {
...
const updateOneUser = async (req, res, next) => {
  ...
  req.file
    ? (updateUser.avatar = [req.file.path, req.file.filename])
    : updateUser;
  ...
  User.findById(req.params.id)
    .then((user) => {
      ...
      if (user.carts.length > 0 && exist) {
        return User.updateOne(
          { _id: user._id, "carts.id_product": req.params.id_product },
          {
            $set: {
              "carts.$.quantity":
                Number(req.query.quantity) + user.carts[key_cart].quantity,
