...
const getSearchPromote = (req, res, next) => {
  Promote.find({ code: req.query.code }).then((promote) => {
    res.json({
      data: promote,
    });
  });
...
const getFilterPromote = (req, res, next) => {
  ...
  // res.json(query)
  Promote.find(query).then((promotesFilter) => {
    res.json({
      data: promotesFilter,
      message: "success",
    });
...
const getPromote = (req, res, next) => {
  Promote.findById(req.params.id).then((promote) => {
    res.json({
      data: promote,
      message: "success",
    });
...
const removePromote = (req, res, next) => {
  Promote.deleteOne({ _id: req.params.id }).then(() => {
    res.status(200).json({
      message: "success",
    });
  });
...
const updatePromote = (req, res, next) => {
  ...
  Promote.updateOne({ _id: req.params.id }, updatePromote).then(() => {
    res.status(200).json({
      message: "success",
    });
  });
